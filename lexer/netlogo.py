...
class NetLogoLexer(RegexLexer):
    ...
    commands = ['__layout-magspring','__set-line-thickness','ask','ask-concurrent','auto-plot-off','auto-plot-on',
                ...
                'with-local-randomness','without-interruption','write','output-dot','inspect-as-reader','layout-grid','set-screen-size']

    reporter = ['breed','abs','acos',r'all\?','and',r'any\?','approximate-hsb','approximate-rgb','asin','atan','at-points',r'autoplot\?',
                'base-colors','behaviorspace-run-number','bf','bl','both-ends','butfirst','but-first','butlast','but-last',
                r'can-move\?','ceiling','color','cos','count','date-and-time','distance','distancexy','dx','dy',r'empty\?','end1',
                'end2','error-message','exp','extract-hsb','extract-rgb',r'file-at-end\?',r'file-exists\?','file-read',
                'file-read-characters','file-read-line','filter','first','floor','fput','heading',r'hidden\?','hsb',
                r'hubnet-enter-message\?',r'hubnet-exit-message\?','hubnet-message','hubnet-message-source','hubnet-message-tag',
                r'hubnet-message-waiting\?','ifelse-value','in-cone','in-link-from',r'in-link-neighbor\?','in-link-neighbors',
                'in-radius','int',r'is-agent\?',r'is-agentset\?',r'is-boolean\?',r'is-directed-link\?',r'is-link\?',r'is-link-set\?',
                r'is-list\?',r'is-number\?',r'is-patch\?',r'is-patch-set\?',r'is-string\?',r'is-turtle\?',r'is-turtle-set\?',
                r'is-undirected-link\?','item','label','label-color','last','length','link','link-heading','link-length',
                r'link-neighbor\?','link-neighbors','links','link-set','link-shapes','link-with','list','ln','log','lput','map',
                'max','max-n-of','max-one-of','max-pxcor','max-pycor','mean','median',r'member\?','min','min-n-of','min-one-of',
                'min-pxcor','min-pycor','mod','modes',r'mouse-down\?',r'mouse-inside\?','mouse-xcor','mouse-ycor','movie-status',
                'my-in-links','my-links','my-out-links','myself','neighbors','neighbors4',r'netlogo-applet\?','netlogo-version',
                'new-seed','n-of','no-links','no-patches','not','no-turtles','n-values','of','one-of','or','other','other-end',
                r'out-link-neighbor\?','out-link-neighbors','out-link-to','patch','patch-ahead','patch-at',
                'patch-at-heading-and-distance','patches','patch-here','patch-left-and-ahead','patch-right-and-ahead',
                'patch-set','patch-size','pcolor','pen-mode','pen-size','plabel','plabel-color','plot-name',r'plot-pen-exists\?',
                ...
                r'shade-of\?','shape','shapes','shuffle','sin','size','sort','sort-by','sqrt','standard-deviation','subject',
                ...
                'user-file','user-input','user-new-file','user-one-of',r'user-yes-or-no\?','variance','who','with','with-max','with-joining','can-hatch\?','extract-hsb',
    ...
    extension_reporter = ['gis:RASTER-SAMPLE','gis:RASTER-WORLD-ENVELOPE','gis:COVERAGE-MINIMUM-THRESHOLD','gis:PROPERTY-MAXIMUM',
                          'gis:LOAD-DATASET',r'gis:INTERSECTS\?','gis:MYWORLD-GET-DATASET',r'gis:CONTAINED-BY\?','gis:HEIGHT-OF',
                          'gis:FIND-ONE-FEATURE','gis:LOCATION-OF','gis:LINK-DATASET','gis:PROPERTY-VALUE','gis:FIND-RANGE',
                          r'gis:CONTAINS\?','gis:PATCH-DATASET','gis:FIND-LESS-THAN','gis:RESAMPLE','gis:TYPE-OF','gis:ENVELOPE-UNION-OF',
                          ...
                          'gis:WORLD-ENVELOPE','gis:ENVELOPE-OF',r'gis:HAVE-RELATIONSHIP\?','gis:MAXIMUM-OF','gis:TURTLE-DATASET',
                          ...
                          'gogo:BURST-VALUE','gogo:PING','gogo:PORTS',r'gogo:OPEN\?','sound:__SYNTH-DUMP','sound:DRUMS',
                          ...
                          'table:MAKE','table:FROM-LIST','table:TO-LIST','table:GET',r'table:HAS-KEY\?','table:KEYS','table:LENGTH',
