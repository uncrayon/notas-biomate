...
\paragraph{Ejercicio 1 (mecánico; clasificación).} Aplicando la \cref{eq:sistema_general} y su definición de linealidad, clasifica cada sistema como lineal o no lineal, señalando el término culpable en los no lineales:
\begin{enumerate}
    \item[(i)] $\dot{x} = -3x + 2y,\quad \dot{y} = x - y$;
    \item[(ii)] $\dot{x} = rx(1-x)$;
    \item[(iii)] $\dot{x} = y,\quad \dot{y} = -\sin x$ (péndulo);
    \item[(iv)] $\dot{x} = a x + b y,\quad \dot{y} = c x + d y$.
\end{enumerate}
\aside{Pista: basta detectar potencias, productos cruzados o funciones trascendentes de las variables de estado.}
% SOL: (i) lineal; (ii) no lineal (termino x^2); (iii) no lineal (sin x); (iv) lineal.

\paragraph{Ejercicio 2 (análisis; superposición).} Demuestra que si $\vec{x}(t)$ y $\vec{y}(t)$ son dos soluciones del sistema lineal $\dot{\vec{x}}=A\vec{x}$, entonces cualquier combinación $\alpha\vec{x}(t)+\beta\vec{y}(t)$ también lo es. Explica en una frase por qué este argumento \emph{falla} para $\dot{x}=x^2$.
\aside{Pista: usa la linealidad de la derivada y del producto matriz--vector. Para $x^2$, $(\alpha x)^2\neq\alpha x^2$.}
% SOL: d/dt(a x+b y)=a A x + b A y = A(a x+b y). Falla en x^2 porque el cuadrado no respeta la superposicion.

\paragraph{Ejercicio 3 (interpretación; universalidad de Feigenbaum).} En el mapa logístico las tres primeras duplicaciones de periodo ocurren aproximadamente en $r_1=3.000$, $r_2=3.449$, $r_3=3.544$. Estima la constante de Feigenbaum con $\delta\approx (r_2-r_1)/(r_3-r_2)$ y compárala con el valor universal $\delta\approx 4.669$. ¿Qué tiene de asombroso que este número aparezca también en sistemas físicos totalmente distintos?
\aside{Pista: $(3.449-3.000)/(3.544-3.449)$. La universalidad significa independencia de los detalles del sistema.}
% SOL: delta ~ 0.449/0.095 ~ 4.73, cercano a 4.669. Asombroso: mismo numero en fluidos, circuitos, etc., independiente de los detalles microscopicos (renormalizacion).
