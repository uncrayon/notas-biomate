...
\paragraph{Ejercicio 1 (mecánico; linealización).} Para $\dot{x} = x - x^3$, halla los tres puntos fijos, evalúa $f'(x^*)$ en cada uno, clasifícalos por el criterio del signo y reporta el tiempo característico $\tau=1/|f'(x^*)|$ de los hiperbólicos.
\aside{Pista: $f'(x)=1-3x^2$; evalúa en $0$ y $\pm 1$.}
% SOL: FP 0,+1,-1. f'(0)=1>0 inestable, tau=1. f'(±1)=-2<0 estables, tau=1/2.

\paragraph{Ejercicio 2 (análisis; caso no hiperbólico).} Para $\dot{x} = x^2(1-x)$, muestra que en $x^*=0$ la linealización es inconclusa ($f'(0)=0$) y determina la estabilidad por el signo de $f$ a ambos lados. Clasifica también $x^*=1$ con el criterio lineal y da su $\tau$.
\aside{Pista: cerca de $0$, $f(x)\approx x^2\ge 0$ a ambos lados: nodo semi-estable. En $x=1$, $f'(x)=2x-3x^2$.}
% SOL: x=0: f'(0)=0, f>=0 a ambos lados -> semi-estable (atrae por izq, repele por der). x=1: f'(1)=2-3=-1<0 estable, tau=1.

\paragraph{Ejercicio 3 (interpretación; ritmos imposibles en 1D).} Un estudiante propone modelar la concentración de una hormona con oscilación circadiana mediante una única ecuación autónoma $\dot{x}=f(x)$ sobre $\mathbb{R}$. Explica por qué esto es imposible y enuncia dos modificaciones estructurales mínimas que sí permitirían un ritmo periódico.
\aside{Pista: recuerda el teorema de no oscilación en $\mathbb{R}$. Opciones: cambiar el espacio de fases a $S^1$, o añadir una segunda variable de estado.}
% SOL: Un flujo autonomo en R es monotono -> no puede oscilar. Soluciones: (i) fase en S^1 con thetadot=omega; (ii) pasar a un sistema 2D (p.ej. anadir una variable lenta acoplada).
